...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, ResponsiveContainer, Legend, Line, LineChart, Tooltip } from 'recharts';
...
function App() {
  ...
  const trendsCurrent = generateMockTrends(period);
  const trendsCompare = generateMockTrends(compareWith);
          ...
          <Box sx={{ flexGrow: 1, p: { xs: 2, md: 4 }, bgcolor: '#f5f6fa', minHeight: 0 }}>
            ...
            <Grid container spacing={4} sx={{ mb: 4, display: 'flex', alignItems: 'center' }}>
              <Grid item xs={12} md={4} sx={{ display: 'flex', flex: 1 }}>
                <OverviewCard
                  title="ADR vs Avg Compset"
                  value={`$${kpiCurrent.ADR} vs $${kpiCurrent.compsetADR}`}
              ...
              <Grid item xs={12} md={4} sx={{ display: 'flex', flex: 1 }}>
                <OverviewCard
                  ...
                  explanation="A high score means your rates are consistent across channels."
            ...
            {/* Rate Trends Card */}
            <Card sx={{ mt: 2, mb: 4, bgcolor: '#fff', borderRadius: 3, boxShadow: '0 2px 12px 0 rgba(16,30,54,0.04)', border: '1px solid #f0f1f3' }}>
              <CardContent>
                ...
                <ResponsiveContainer width="100%" height={320}>
                  {chartView === 'bar' ? (
                    <BarChart data={trendsCurrent} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                      ...
                      <XAxis dataKey="day" tick={{ fontWeight: 600, fontSize: 12 }} axisLine={false} tickLine={false} />
                      <YAxis tick={{ fontWeight: 600, fontSize: 14 }} axisLine={false} tickLine={false} />
                      <Tooltip
                        content={({ active, payload, label }) => {
                          if (active && payload && payload.length) {
                            const data = payload[0].payload;
                            return (
                              <Box p={2} bgcolor="#fff" borderRadius={2} boxShadow={4} minWidth={220}>
                                <Typography variant="subtitle2" fontWeight={700} mb={0.5} color="#1976d2">{label}</Typography>
                                ...
                                <Typography variant="body2" color="primary.main" fontWeight={700} mt={1}>Position: {data.position}</Typography>
                              </Box>
                            );
                          }
                          return null;
                      ...
                      <Bar dataKey="ADR" name="ADR" fill="url(#colorADR)" radius={[12, 12, 0, 0]} barSize={8}
                        ...
                        onMouseOver={e => e && (e.target.style.filter = 'drop-shadow(0 2px 8px #1976d233)')}
                      ...
                      <Bar dataKey="Compset" name="Avg Compset" fill="url(#colorCompset)" radius={[12, 12, 0, 0]} barSize={8}
                        style={{ transition: 'filter 0.2s' }}
                        onMouseOver={e => e && (e.target.style.filter = 'drop-shadow(0 2px 8px #9c27b033)')}
                        onMouseOut={e => e && (e.target.style.filter = '')}
                      />
                    </BarChart>
                  ) : (
              ...
              <CardContent>
                ...
                <Grid container spacing={3}>
                  {distCurrent.map((channel, idx) => (
                    <Grid item xs={12} md={4} key={channel.name}>
                      <Card
                        variant="outlined"
                        sx={{
                          borderRadius: 2,
